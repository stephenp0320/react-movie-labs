...
export const getMovies = () => {
  ...
  export const getWatchlist = () => {
    ...
    const { id } = idPart;
    return fetch(
      `https://api.themoviedb.org/3/account/${id}/watchlist/movies?api_key=${import.meta.env.VITE_TMDB_KEY}&language=en-US&include_adult=false&include_video=false&page=1`
    ).then((response) => {
      if (!response.ok) {
        return response.json().then((error) => {
          throw new Error(error.status_message || "Something went wrong");
  ...
  export const getKeyword = ({ queryKey }) => {
    ...
    const { id } = idPart;
    return fetch(
      `https://api.themoviedb.org/3/movie/${id}/keywords?api_key=${import.meta.env.VITE_TMDB_KEY}`
    ).then( (response) => {
      if (!response.ok) {
        return response.json().then((error) => {
          throw new Error(error.status_message || "Something went wrong");
...
export const getMovieImages = ({ queryKey }) => {
  ...
  export const getAlternativeTitles = ({ queryKey }) => {
    ...
    return fetch(
      `https://api.themoviedb.org/3/movie/${id}/alternative_titles?api_key=${import.meta.env.VITE_TMDB_KEY}`
    )
      .then((response) => {
        if (!response.ok) {
          return response.json().then((error) => {
